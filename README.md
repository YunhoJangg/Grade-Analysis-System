...
CSV 파일 형태의 학생 성적 데이터를 불러와 통계 분석을 수행하고, 시각화 그래프를 포함한 전문적인 PDF 리포트를 자동으로 생성하는 프로그램입니다.

## 1. 주요 기능 (Key Features)
- **데이터 로드 및 검증**: CSV 파일을 읽어오며, 점수 범위(0-100) 및 데이터 타입의 유효성을 자동으로 검증합니다.
- **통계 분석**: 각 학생의 총점/평균 계산 및 학급 전체의 평균, 중앙값, 최댓값, 최솟값, 상위 25% 커트라인을 산출합니다.
...
- `main.py`: 프로그램 실행 및 GUI 인터페이스 제어  
...
## 5. 요구사항 대응 현황 (Requirement Mapping)

1. CSV 불러오기
    - Pandas를 이용한 데이터 로딩 (data_manager.py)
2. 데이터 검증
    - 점수 범위(0-100) 및 타입 체크 (data_manager.py)
3. 통계 분석 기능
    - 학생별/학급 전체 통계 산출 (analyzer.py)
4. 시각화 기능
    - 3종의 그래프(막대, 분포, 추이) 생성 (visualizer.py)
5. PDF 리포트 생성
    - 통계 및 이미지를 포함한 PDF 제작 (report_generator.py)
6. 분석 결과 저장
    - 가공된 데이터를 새로운 CSV로 저장 (main.py)
7. 예외 처리
    - 상황별 오류 안내 메시지 제공 (data_manager.py)
8. 사용성
    - tkinter 기반 GUI 인터페이스 제공 (main.py)
9. 확장성
    - 기능별 독립 모듈 구조 설계
